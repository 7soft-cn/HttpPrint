        ...
        public void Print(Graphics g)
        {
            if (this.PrintInfos != null && this.PrintInfos.Count > 0)
            {
                ...
                foreach (PrintItem p in this.PrintInfos)
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(p.Value) && p.Value.Contains($"{{{item.Key}}}"))
                        {
                            p.Value = p.Value.Replace($"{{{item.Key}}}", item.Value);
                        }
                    }
                    ...
                    switch (p.PrtType)
                    {
                        case PrintType.Text:
                            {
                                ...
                                g.DrawString(p.Value, tFont, b, p.Start);
                            }
                            break;
                        ...
                        case PrintType.QrCode:
                            {
                                var img = NewQRCodeByZxingNet(p.Value, p.Size, p.Size);
                                g.DrawImage(img, p.Start);
                            }
                            break;
                        case PrintType.BarCode:
                            {
                                var img = NewBarCodeByZxingNet(p.Value, p.Width, p.Height, BarcodeFormat.CODE_128);
                                g.DrawImage(img, p.Start);
                            }
                            break;
                        default:
